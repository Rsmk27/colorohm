export class ResistorCalculator {
  constructor() {
    ...
    for (const [valueCode, baseValue] of Object.entries(this.eia96Values)) {
      for (const [multiplierCode, multiplier] of Object.entries(
        this.eia96Multipliers,
      )) {
  ...
  getColorsForType(type) {
    return Object.keys(this.colorCodes).filter(
      (color) =>
        this.colorCodes[color][type] !== null &&
        this.colorCodes[color][type] !== undefined,
    );
  }
  ...
  calculateFromColors(colors, bandCount) {
    try {
      ...
      return {
        ...
        formattedValue:
          this.formatResistance(resistanceValue) + ` ±${tolerance}%`,
  ...
  predictColors(resistanceOhms, tolerance, preferredBandCount = 4) {
    try {
      ...
      const preferredResults = results.filter(
        (r) => r.bandCount === preferredBandCount,
      );
      const bestResult =
        preferredResults.length > 0 ? preferredResults[0] : results[0];
  ...
  findAllPossibleColors(targetResistance, tolerance) {
    ...
    for (const bandCount of [4, 5]) {
      ...
      for (const mult of multipliers) {
        ...
        if (
          significantValue >= Math.pow(10, digitCount - 1) &&
          ...
          if (validCombination) {
            ...
            colors.push(toleranceColor); // Best tolerance color

            const calculatedValue = rounded * mult.value;
            const accuracy =
              100 -
              (Math.abs(calculatedValue - targetResistance) /
                targetResistance) *
                100;
            ...
            results.push({
              ...
              formattedValue:
                this.formatResistance(calculatedValue) + ` ±${tolerance}%`,
  ...
  decodeEIA96SMD(code) {
    ...
    if (!/^\d{2}[A-Z]$/.test(code)) {
      return {
        success: false,
        error: "Invalid EIA-96 code format (should be like 01A)",
      };
    }
  ...
  encode3DigitSMD(resistanceOhms) {
    ...
    if (!bestCode) {
      return {
        success: false,
        error: "Value cannot be represented with 3-digit SMD code",
      };
    }
  ...
  encode4DigitSMD(resistanceOhms) {
    ...
    if (!bestCode) {
      return {
        success: false,
        error: "Value cannot be represented with 4-digit SMD code",
      };
    }
  ...
  encodeEIA96SMD(resistanceOhms) {
    ...
    if (!bestMatch) {
      return {
        success: false,
        error: "Value cannot be represented with EIA-96 code",
      };
    }
